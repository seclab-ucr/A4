...
{
    AdGraph::AdGraph(std::string base_domain) : base_domain_(base_domain) {}
    ...
    AdGraphAPI::HTTPNode *AdGraph::CreateAndReturnHTTPNode(std::string id, bool script_is_active, std::string active_script_id, std::string url, bool ad)
    {
        HTTP_node_ids_.insert(id);
        AdGraphAPI::HTTPNode *http_node(new HTTPNode(id, script_is_active, active_script_id, url, ad));
    ...
    void AdGraph::RemoveNode(std::string id)
    {
        node_ids_.erase(id);
    }

    void AdGraph::AddEdge(std::string source_id, std::string target_id)
    {
        edge_list_.push_back(std::make_pair(source_id, target_id));
    }

    void AdGraph::RemoveEdge(std::string source_id, std::string target_id)
    {
        edge_list_.erase(std::remove(edge_list_.begin(), edge_list_.end(), std::make_pair(source_id, target_id)), edge_list_.end());
    }
    ...
    bool AdGraph::VerifyFirstParentPropertiesObject(Utilities::ordered_json json_object)
    {
        return json_object[FEATURE_FIRST_PARENT_ASYNC].get<bool>() &&
                       ...
                       (json_object[FEATURE_FIRST_PARENT_TAG_NAME].get<std::string>() == "")
                   ? false
                   : true;
    }
    ...
    bool AdGraph::VerifySecondParentPropertiesObject(Utilities::ordered_json json_object)
    {
        return json_object[FEATURE_SECOND_PARENT_ASYNC].get<bool>() &&
                       ...
                       (json_object[FEATURE_SECOND_PARENT_TAG_NAME].get<std::string>() == "UNKNOWN")
                   ? false
                   : true;
    }
    ...
    Utilities::ordered_json AdGraph::GetNodeProperties(AdGraphAPI::HTTPNode *http_node, std::string event_type)
    {
        ...
        json_object[FEATURE_REQUEST_URL] = http_node->GetURL();
    ...
    Utilities::ordered_json AdGraph::PrepareJSONVisualization()
    {
        ...
        for (const auto &edge : edge_list_)
        {
            ...
            json_obj["source"] = edge.first;
            json_obj["target"] = edge.second;
